...
Para mantener la cordura y el paradigma *Atomic Design*, el **Document Designer** no será un archivo monolítico de 2000 líneas. Se modularizará estrictamente en el siguiente árbol de directorios dentro de `components/macro_engines/DocumentDesigner/`:

```text
client/src/components/macro_engines/DocumentDesigner/
│
├── DocumentDesigner.jsx            # [ORQUESTADOR] Punto de entrada. Monta el Layout Flexbox base (HUD, Paneles, Lienzo) e hidrata el AST.
│
├── hooks/
│   ├── useDocumentAST.js           # [ESTADO] Hook puro que gestiona las mutaciones del árbol (Move, Add, Delete, Update Node).
│   └── useDocumentHydration.js     # [DATOS] Conecta con el Core, lee el de la Columna IN y expone los Slots disponibles.
│
├── layout/
│   ├── TopRibbon.jsx               # [HUD] Barra superior. Contiene el bloque de Inserción (Draggable Atomics) y el botón SYNCING.
│   ├── LeftPanel.jsx               # [IN/SOURCES] Gestor de pestañas: Data Sources (PortManager) y el Layer Tree.
│   ├── LayerTree.jsx               # [JERARQUÍA] Componente recursivo que pinta el índice de capas 1D (Estilo Figma) a la izquierda.
│   └── RightPanel.jsx              # [INSPECTOR] Contenedor del Inspector Paramétrico mutante.
│
├── renderer/
│   ├── CanvasMonitor.jsx           # [LIENZO] Área central. Solo recibe el AST y pinta el primer `RecursiveBlock`.
│   └── RecursiveBlock.jsx          # [MOTOR] El renderizador mágico. Lee el `block.type` y envuelve a los sub-componentes en contenedores estilo AutoLayout puros.
│
├── blocks/                         # [ÁTOMOS] Los componentes terminales (Nodos). Cada uno obedece el Layout de su padre.
│   ├── FrameBlock.jsx              # Contenedor Flexbox estándar. Renderiza a sus `children`.
│   ├── TextBlock.jsx               # Renderizador de texto con sub-motor de Regex para `{{slots}}`.
│   ├── ImageBlock.jsx              # Contenedor responsive `object-fit`.
│   └── IteratorBlock.jsx           # Frame especializado que clona en loop su contenido usando los datos del Sandbox IN.
│
└── inspector/                      # [CONTROLES DERECHOS] Dials paramétricos puros.
    ├── LayoutControls.jsx          # Matriz 3x3 de alineación, selects de Hug/Fill, Dials para Padding/Gap.
    ├── TextControls.jsx            # Tipografía, Pesos, Color, Botón de inyección de Slots.
    └── SourceSelectorControls.jsx  # Para vincular un ITERATOR o FRAME a un array del IN.
```
