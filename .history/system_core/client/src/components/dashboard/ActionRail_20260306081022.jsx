...
export function ActionRail({ onCreate }) {
    ...
    const creatableEngines = ENGINE_MANIFESTS.filter(e => e.canCreate);
    if (creatableEngines.length === 0) return null;
        ...
        <div className="action-rail shelf" style={{
            ...
            {creatableEngines.map(engine => (
                ...
                >
                    ...
                    <span style={{ fontSize: '10px', fontFamily: 'var(--font-mono)' }}>{t(engine.label) || engine.label}</span>
                </button>
            ))}
        </div>
    );
