...
// Diseño determinista basado en clase (Cero Burocracia)
const cardThemes = {
...
return (
    ...
    >
        ...
        <div className="spread" style={{ marginTop: 'auto', borderTop: '1px solid var(--color-border)', paddingTop: 'var(--space-2)' }}>
            <div className="shelf--tight" onClick={e => e.stopPropagation()}>
                {activeActions.map(protocol => {
                    ...
                    // Protección Agnóstica de Infraestructura: 
                    // No permitir borrado de Silos/Cuentas desde el Escenario si son CORE.
                    if (engine?.isInfrastructure && config.requiresHold) return null;
        ...
        <style>{`
                ...
                .mca-surface:hover {
                    border-color: ${engine?.color || 'var(--color-accent)'} !important;
                    box-shadow: 0 4px 20px rgba(0,0,0,0.4);
                    transform: translateY(-2px);
                }
                .mca-surface:hover h3 {
                    color: ${engine?.color || 'white'} !important;
                }
            `}</style>
    </div>
);
