...
export function ArtifactGrid({ pins, onOpen }) {
    const lang = useAppState(s => s.lang);
    const t = useLexicon(lang);

    // 1. Agrupar pines por clase conocida (Motores registrados)
    const pinsList = pins || [];
    const groups = ENGINE_MANIFESTS.map(engine => {
        return {
            ...engine,
    ...
    // 2. Capturar átomos huérfanos (sin motor registrado) - Radical Agnóstico
    const knownClasses = ENGINE_MANIFESTS.map(m => m.class);
    const orphanItems = pinsList.filter(p => !knownClasses.includes(p.class));

    if (orphanItems.length > 0) {
        groups.push({
            class: 'UNKNOWN_CLASS',
            label: 'UNCATEGORIZED_ATOMS',
                ...
                <section key={group.class} className="stack" style={{ gap: 'var(--space-4)' }}>
                    ...
                    <div className="grid-auto">
                        {group.items.map(atom => (
                            <ArtifactCard key={`${atom.provider}:${atom.id}`} atom={atom} onOpen={onOpen} />
                        ))}
                    </div>
                </section>
            ))}
