...
export function ArtifactGrid({ pins, onOpen, onAction }) {
    ...
    // 1. Filtrar solo artefactos operativos (No infraestructura)
    const pinsList = (pins || []).filter(p => {
        const engine = getEngineForClass(p.class);
        return engine && !engine.isInfrastructure;
    });
