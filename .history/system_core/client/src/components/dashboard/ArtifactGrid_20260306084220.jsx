...
export function ArtifactGrid({ pins, onOpen, onAction }) {
    ...
    // 1. Filtrar solo artefactos operativos (Excluir Infraestructura CORE por clase)
    const pinsList = (pins || []).filter(p => !['FOLDER', 'ACCOUNT_IDENTITY'].includes(p.class));

    // 2. Agrupar por Clase (Determinismo Dinámico)
    const uniqueClasses = [...new Set(pinsList.map(p => p.class))];

    const groups = uniqueClasses.map(cls => {
        const items = pinsList.filter(p => p.class === cls);
        // Configuración visual por asignación semántica simple
        let config = { color: 'var(--color-accent)', label: cls };
        if (cls === 'DATA_SCHEMA') config = { color: 'var(--color-accent)', label: 'DATA_SCHEMA' };
        if (cls === 'BRIDGE') config = { color: 'var(--color-cold)', label: 'LOGIC_BRIDGE' };
        if (cls === 'DOCUMENT') config = { color: 'var(--color-warm)', label: 'DOCUMENT_TEMPLATE' };
