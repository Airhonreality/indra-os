...
export function BridgeDesigner({ atom, bridge }) {
    ...
    const getContextAt = (index) => {
        ...
        (localAtom.payload?.operators || []).slice(0, index).forEach(op => {
            if (op.alias) {
                context.ops[op.alias] = { type: 'UNKNOWN', result: null };
            }
        });
    ...
    const updatePortConfig = (id, newConfig, type) => {
        ...
        pushToHistory({
            ...
            payload: {
                ...
                [key]: {
                    ...(prev.payload[key] || {}),
                    [id]: { ...(prev.payload[key]?.[id] || {}), ...newConfig }
                }
            }
        });
    };
    ...
    return (
        <div className="stack" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'var(--color-bg-void)', color: 'white', overflow: 'hidden', zIndex: 100 }}>
            ...
            <header className="spread glass" style={{ padding: 'var(--space-2) var(--space-6)', borderBottom: '1px solid var(--color-border-strong)', zIndex: 100 }}>
                ...
                <div className="shelf">
                    ...
                    <div className="shelf--tight" style={{ marginRight: 'var(--space-4)' }}>
                        <button className="btn btn--xs btn--ghost" onClick={undo} disabled={pointer === 0} style={{ opacity: pointer === 0 ? 0.3 : 1 }}>
                            <IndraIcon name="ARROW_LEFT" size="12px" />
                        </button>
                        <button className="btn btn--xs btn--ghost" onClick={redo} disabled={pointer === history.length - 1} style={{ opacity: pointer === history.length - 1 ? 0.3 : 1 }}>
                            <IndraIcon name="ARROW_RIGHT" size="12px" />
                        </button>
                    </div>
            ...
            {/* OPERATOR TOOLBAR */}
            <div className="shelf glass" style={{ padding: 'var(--space-1) var(--space-6)', borderBottom: '1px solid var(--color-border)', background: 'rgba(0,0,0,0.3)', minHeight: '32px' }}>
                ...
                <div className="shelf--tight" style={{ gap: 'var(--space-2)' }}>
                    {['MATH', 'TEXT', 'RESOLVER', 'EXPRESSION'].map(type => (
                        <button key={type} className="shelf--tight btn btn--xs btn--ghost" onClick={() => addOperator(type)} style={{ borderRadius: 'var(--radius-pill)', fontSize: '10px', opacity: 0.8, padding: '2px 8px' }}>
                            <IndraIcon name="PLUS" size="8px" /> {type}
                        </button>
                    ))}
                </div>
            </div>
