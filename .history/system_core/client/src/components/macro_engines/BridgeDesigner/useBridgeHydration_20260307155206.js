        ...
        allExternalIds.forEach(id => {
            ...
            const fetchSchema = async () => {
                try {
                    ...
                    // ADR_001 Manda que TABULAR_STREAM devuelva los headers en metadata.schema.columns
                    const schemaColumns = result.metadata?.schema?.columns || result.metadata?.schema?.fields || result.fields;
                    ...
                    if (schemaColumns) {
                        setSchemas(prev => ({
                            ...
                            [id]: {
                                ...
                                label: result.metadata?.schema?.name || result.handle?.label || result.label || 'EXTERNAL_SILO'
                            }
                        }));
                    } else if (result.metadata?.status === 'ERROR') {
                        setSchemas(prev => ({
                            ...prev,
                            [id]: {
                                fields: [],
