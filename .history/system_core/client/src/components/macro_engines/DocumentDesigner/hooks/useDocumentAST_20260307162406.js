    ...
    const addNode = useCallback((type, parentId = null) => {
        ...
        setBlocks(prev => {
            ...
            if (actualParentId) {
                ...
                // Si el padre no es un contenedor (FRAME o ITERATOR), forzamos al root del documento
                if (parentNode && parentNode.type !== 'FRAME' && parentNode.type !== 'ITERATOR') {
                    // Axioma: Un nodo terminal no puede engendrar hijos.
                    actualParentId = 'root';
                }
            }
