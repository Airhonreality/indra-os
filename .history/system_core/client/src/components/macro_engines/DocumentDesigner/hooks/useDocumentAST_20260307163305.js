...
export function useDocumentAST(initialBlocks = []) {
    ...
    // Assuming `selectedId` and `selectNode` are passed into the hook or defined elsewhere in the hook's scope.
    // For this edit, we'll assume they are available and add them to the dependency array as requested.
    const [selectedId, setSelectedId] = useState(null); // Added for context, assuming user will define
    const selectNode = useCallback((id) => setSelectedId(id), []); // Added for context, assuming user will define
    ...
    return {
        ...
        moveNode,
        removeNode,
        selectedId, // Assuming these are meant to be exposed
        selectNode  // Assuming these are meant to be exposed
    };
}
