...
export function LayersPanel({ fields, selectedId, onSelect, onAdd, onRemove, onMove, onClone, onDemote, onPromote }) {
    ...
    return (
        <aside className="stack" style={{
            ...
            {/* Header del Panel */}
            <div style={{ flexShrink: 0, padding: 'var(--space-4) var(--space-6)', borderBottom: '1px solid var(--color-border)' }}>
                <div className="spread">
                    <span style={{ fontSize: '10px', fontFamily: 'var(--font-mono)', fontWeight: 'bold', opacity: 0.6 }}>DNA_LAYERS</span>
                    <div className="shelf--tight">
                        <button className="btn btn--ghost btn--sm" onClick={() => onAdd('TEXT')} title="Añadir Campo Raíz">
                            <IndraIcon name="PLUS" size="12px" />
                        </button>
                        <button className="btn btn--ghost btn--sm" onClick={() => onAdd('FRAME')} title="Añadir Frame Raíz">
