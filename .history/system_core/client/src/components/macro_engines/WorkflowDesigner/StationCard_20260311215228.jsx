...
export function StationCard({ station, index, isSelected, isExecuting, onSelect, isOrphan }) {
    ...
    return (
        <div
            className={`station-node type-${station.type.toLowerCase()} ${isSelected ? 'selected' : ''} ${isExecuting ? 'executing' : ''} ${isCabled() ? 'cabled' : 'uncabled'} ${isOrphan ? 'is-orphan' : ''}`}
            onClick={onSelect}
            style={isOrphan ? { borderColor: 'var(--color-danger)', borderStyle: 'dashed' } : {}}
        >
            ...
            <div className="node-content fill">
                <div className="shelf--tight" style={{ marginBottom: '4px' }}>
                    ...
                    {isOrphan && (
                        <div className="cabling-status uncabled shelf--tight" style={{ background: 'var(--color-danger)' }}>
                            <IndraIcon name="WARNING" size="8px" />
                            <span style={{ fontSize: '7px' }}>MATERIA_DESAPARECIDA</span>
                        </div>
                    )}
                    {isCabled() && !isOrphan ? (
