    ...
    return (
        <aside className="engine-sidebar inspector-panel">
            <header className="panel-header spread">
                <div className="shelf--tight">
                    <div style={{ color: `var(--color-${station.type.toLowerCase() === 'protocol' ? 'accent' : station.type.toLowerCase() === 'router' ? 'warm' : 'cold'})` }}>
                        <IndraIcon name={station.type} size="14px" />
                    </div>
                    <h3 className="font-mono">{station.id.replace('step_', 'STEP_')} // INSPECT</h3>
                </div>
                <button onClick={() => setSelectedStationId(null)} className="hud-btn-close">
                    ...
                    <section className="stack">
                        ...
                        <div className="stack--tight pruning-container glass-void">
                            {Object.entries(buildContextStack().sources).map(([alias, schema]) => (
                                <div key={alias} className="shelf--tight pruning-item glass-hover">
