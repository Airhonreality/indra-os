        ...
        <div className={`workflow-sandbox glass ${status === 'RUNNING' ? 'running' : ''}`}>
            <header className="spread">
                <div className="shelf--tight">
                    <span className="text-label" style={{ fontSize: '9px', opacity: 0.5 }}>SANDBOX // {status.toUpperCase()}</span>
                </div>
                <div className="shelf">
                    <button
                    ...
                    >
                        ...
                        {status === 'RUNNING' ? 'TRACE_RUNNING...' : 'RUN_TEST_TRACE'}
                    </button>
                </div>
            </header>
            ...
            <main className="sandbox-grid">
                <div className="mock-area stack--tight">
                    <div className="shelf--tight" style={{ marginBottom: '4px' }}>
                        ...
                        <span className="text-label" style={{ fontSize: '8px', opacity: 0.6 }}>MOCK_INPUT_DNA</span>
                    </div>
                ...
                <div className="trace-area stack--tight">
                    <div className="shelf--tight" style={{ marginBottom: '4px' }}>
                        ...
                        <span className="text-label" style={{ fontSize: '8px', opacity: 0.6 }}>SYSTEM_EXECUTION_TRACE (GAS_RESONANCE)</span>
                    </div>
                    <div className="terminal-box glass-void stack--tight" style={{ minHeight: '120px' }}>
                        {traceLogs.length === 0 && (
                            <div className="trace-line shelf--tight font-mono opacity-20 center">
                                <span className="text-hint">READY_TO_SIMULATE_PIPELINE...</span>
                            </div>
                        )}
                        {traceLogs.map((log, i) => (
                        ...
                        {status === 'RUNNING' && <div className="blink-line" style={{ width: '8px', height: '2px', background: 'var(--color-accent)', marginTop: '4px' }} />}
                    </div>
                </div>
            </main>
        </div>
