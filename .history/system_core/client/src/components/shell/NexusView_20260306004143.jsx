...
export function NexusView() {
    ...
    // Mock de workspaces (Simulando SYSTEM_WORKSPACES_LIST)
    const [workspaces] = useState([
        { id: 'ws_1', handle: { label: 'Operaciones_2024', alias: 'ops_24' }, pins: 12, last_sync: '2h ago' },
        { id: 'ws_2', handle: { label: 'Laboratorio_I+D', alias: 'lab_id' }, pins: 5, last_sync: '5m ago' },
        { id: 'ws_3', handle: { label: 'Estrategia_Global', alias: 'global_strat' }, pins: 8, last_sync: '1d ago' }
    ]);

    // Mock de servicios
    const [services] = useState([
        { id: 'notion', label: 'Notion_Cloud', status: 'OK' },
        { id: 'drive', label: 'Google_Drive', status: 'OK' },
        { id: 'calendar', label: 'Indra_Calendar', status: 'WAITING' }
    ]);
                        ...
                        <div className="stack--tight">
                            {services.map(svc => (
                                <div key={svc.id} className="shelf split" style={{
                                    ...
                                    <span style={{ fontSize: 'var(--text-xs)', fontFamily: 'var(--font-mono)' }}>{svc.label}</span>
                                    <div className="shelf" style={{ gap: 'var(--space-2)' }}>
                                        <div style={{
                                            width: '6px', height: '6px',
                                            borderRadius: '50%',
                                            background: svc.status === 'OK' ? 'var(--color-accent)' : 'var(--color-warm)',
                                            boxShadow: svc.status === 'OK' ? '0 0 5px var(--color-accent)' : '0 0 5px var(--color-warm)'
                                        }}></div>
                                        <span style={{ fontSize: '9px', opacity: 0.5 }}>{svc.status}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <main className="stack" style={{ gap: 'var(--space-6)' }}>
                    ...
                    <div className="grid-auto">
                        {workspaces.map(ws => (
                            ...
                            >
                                <div className="spread">
                                    <IndraIcon name="WORKSPACE" style={{ opacity: 0.4 }} />
                                    <span className="text-hint" style={{ fontSize: '9px' }}>{ws.last_sync}</span>
                                </div>
