...
import { directiveExecutor } from '../../api/core/directive_executor.js';
...
export default function ArtifactSelector({
    ...
    const { manifest, pins, addPin, removePin, isPinned, activeWorkspaceId } = useAppState();
    ...
    const [counts, setCounts] = useState({ PINS: pins.length, ALL: 0, DOCUMENT: 0, TABULAR: 0, FOLDER: 0 });
    ...
    const loadLevel = useCallback(async () => {
        ...
        try {
            if (!currentContext) {
                ...
                const { items: result } = await directiveExecutor.execute({
    ...
    const handleSearchChange = (e) => {
        ...
        searchDebounce.current = setTimeout(async () => {
            setLoading(true);
            try {
                const { items: results } = await directiveExecutor.execute({
                    provider: currentContext.provider,
