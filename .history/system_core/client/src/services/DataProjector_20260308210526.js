...
export class DataProjector {
    ...
    static projectSchema(schemaSource) {
        ...
        // 1. Detección Ágil de campos (Pipeline de Vacío)
        let rawFields = [];
        let label = 'UNTITLED_SCHEMA';
        let id = null;

        if (Array.isArray(schemaSource)) {
            // Caso: El protocolo devolvió directamente el listado
            rawFields = schemaSource;
        } else if (schemaSource.items && Array.isArray(schemaSource.items)) {
            // Caso: Envoltorio del Core (result.items)
            rawFields = schemaSource.items[0]?.payload?.fields || schemaSource.items[0]?.fields || [];
            id = schemaSource.items[0]?.id;
            label = schemaSource.items[0]?.handle?.label || schemaSource.items[0]?.label || label;
        } else {
            // Caso: Átomo o metadata.schema
            id = schemaSource.id;
            label = schemaSource.handle?.label || schemaSource.label || label;
            rawFields = schemaSource.metadata?.schema?.columns ||
                schemaSource.metadata?.schema?.fields ||
                schemaSource.payload?.fields ||
                schemaSource.payload?.layers || // Soporte para capas de diseño
                schemaSource.fields ||
                schemaSource.columns || [];
        }

        // 2. Proyección Recursiva
        const fields = rawFields.map(f => this.projectFieldDefinition(f)).filter(f => f !== null);

        return {
            id: id || schemaSource.id,
            label: label,
            fields: fields,
            raw: schemaSource
        };
