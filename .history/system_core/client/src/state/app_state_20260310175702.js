...
export const useAppState = create((set, get) => ({
    ...
    unpinAtom: async (atomId, provider) => {
        ...
        // --- OPTIMISTIC UNPIN (ADR-003 §3) ---
        // Eliminamos de inmediato en local para evitar el 'Ghosting' por latencia de Drive
        ...
        try {
            ...
            // Recarga de fondo para sincronizar (silenciosa)
            get().loadPins();
        } catch (err) {
            console.error('[app_state] unpinAtom failed, reverting:', err);
            set({ pins: previousPins }); // Revertir si falla el servidor
            throw err;
        }
    },
