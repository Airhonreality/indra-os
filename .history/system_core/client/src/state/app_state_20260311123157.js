...
export const useAppState = create((set, get) => ({
    ...
    deleteArtifact: async (atomId, provider) => {
        ...
        try {
            ...
            try {
                ...
                // No es fatal: el backend también purga en ATOM_DELETE
    ...
    renameWorkspace: async (workspaceId, newLabel) => {
        ...
        // Actualización optimista inmediata en el store
        const updatedWorkspaces = workspaces.map(w => {
            if (w.id !== workspaceId) return w;
            return { ...w, handle: { ...w.handle, label: newLabel } };
        });
    ...
    deleteWorkspace: async (workspaceId) => {
        ...
        try {
            ...
            // Re-hidratar manifiesto sin reiniciar toda la sesión
            get().hydrateManifest();
        } catch (err) {
