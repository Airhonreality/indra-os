...
function _validateAtomContract_(items, providerId) {
  ...
  items.forEach((item, index) => {
    // 1. Aplicar Shadow Identity Protocol (Hidratación de Identidad)
    _hydrateIUH_(item, providerId);

    // 2. Validar campos estructurales obligatorios
    const REQUIRED_BASE = ['id', 'class'];
    const missing = REQUIRED_BASE.filter(field => !item[field]);
    if (missing.length > 0 || !item.handle) {
      throw createError(
        'CONTRACT_VIOLATION',
        `ContractViolation v3.0: El provider "${providerId}" entregó materia no canónica. Faltan campos base: [${missing.join(', ')}].`,
        { item_index: index, item }
      );
    }

    // 3. ADR_008: VALIDACIÓN COERCITIVA POR CLASE
