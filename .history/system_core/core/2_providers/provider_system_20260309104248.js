...
function _system_updateAtom(atomId, updates, providerId) {
  try {
    ...
    // AXIOMA DE PERSISTENCIA (Deep-Merge): Evitar la degradación por sobrescritura de payloads.
    // El sistema protege las claves existentes realizando una unión recursiva.
    ...
    // Campos inmutables por Ley de Identidad
    const { id, class: atomClass, provider, raw, ...pureUpdates } = updates;

    const updated = JSON.parse(JSON.stringify(current)); // Clonar para seguridad

    // 1. Unificar Payload (Zona de Acumulación)
    if (pureUpdates.payload) {
      updated.payload = updated.payload || {};
      deepMerge(updated.payload, pureUpdates.payload);
    ...
    // 3. Unificar el resto de metadatos (updated_at, etc)
    Object.assign(updated, pureUpdates);
    updated.updated_at = new Date().toISOString();

    // 4. Sincronizar nombre del archivo físico si el label cambió
...
function _system_toAtom(doc, fileId, providerId) {
  // ADR-008: Sinceridad Radical con Fallback Legacy para Identidad
  const label = doc.handle?.label || doc.name || 'UNTITLED_ITEM';
  const handle = doc.handle || {
    ns: `com.indra.system.${doc.class?.toLowerCase() || 'item'}`,
    alias: _system_slugify_(label),
    label: label
  };

  const finalPayload = { ...(doc.payload || {}) };

  // ADR-008: Migración y Sinceridad de Esquemas Legacy
  // Si es un esquema y no tiene campos en el payload, intentamos rescatarlos de la raíz (legacy)
  if (doc.class === DATA_SCHEMA_CLASS_ && (!finalPayload.fields || !Array.isArray(finalPayload.fields))) {
    const legacyFields = doc.fields || doc.raw?.fields || [];
    finalPayload.fields = Array.isArray(legacyFields) ? legacyFields : [];
    logDebug(`[provider_system] Migrando ${finalPayload.fields.length} campos legacy para ${fileId}`);
  }

  // Detectar átomos extremadamente rotos (sin clase)
  if (!doc.class) {
    logWarn(`[provider_system] Átomo ${fileId} ignorado por falta de Sinceridad (class faltante).`);
    return {
      id: fileId,
      class: 'BROKEN_ATOM',
      handle: handle,
...
function _system_handleSchemaFieldOptions(uqo) {
  ...
  // 2. Buscar el campo
  const fields = (schemaAtom.payload && schemaAtom.payload.fields) || (schemaAtom.raw && schemaAtom.raw.fields) || [];
...
function _system_handleTabularStream(uqo) {
  ...
  // Si es un esquema de datos, su schema son sus campos configurados
  if (doc.class === DATA_SCHEMA_CLASS_) {
    const rawFields = (doc.payload && doc.payload.fields) || (doc.raw && doc.raw.fields) || [];
    const fields = rawFields.map(f => ({
      id: f.id || f.key,
      label: f.label || f.id || f.key || 'UNTITLED_FIELD',
      type: f.type || 'TEXT'
    }));
