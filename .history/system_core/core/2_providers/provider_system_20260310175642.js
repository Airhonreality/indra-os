...
function handleSystem(uqo) {
  ...
  if (protocol === 'SYSTEM_AUDIT') return runResonanceAudit();
...
function _system_createAtom(atomClass, label, extraData, providerId) {
  try {
    ...
    // AXIOMA ADR-008: Todo DATA_SCHEMA debe nacer con un array de campos.
    if (atomClass === DATA_SCHEMA_CLASS_ && (!initialPayload.fields || !Array.isArray(initialPayload.fields))) {
...
function _system_toAtom(doc, fileId, providerId) {
  ...
  if (!doc.class || !doc.handle?.label) {
    logWarn(`[provider_system] Átomo ${fileId} ignorado por falta de Sinceridad (class/handle faltantes).`);
