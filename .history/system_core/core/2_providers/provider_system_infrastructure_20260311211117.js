...
function _system_batchVerifyExistence_(ids) {
    ...
    // En Indra, la "Existencia" es binaria.
    // Usamos el Iterador de Drive que es más eficiente que getFileById individual.
    ...
    while (files.hasNext()) {
        const file = files.next();
        results[file.getId()] = !file.isTrashed();
    }

    // Los que no aparecieron son falsos
    ids.forEach(id => {
