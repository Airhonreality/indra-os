...
function _projectToSincerity_(legacy, fileId, fileName) {
    ...
    if (evolved.class === 'DATA_SCHEMA') {
        ...
        if (legacy.fields && !evolved.payload.fields) {
            evolved.payload.fields = legacy.fields;
            delete evolved.fields;
        }

        // Si fields no es array, inicializar
        if (!Array.isArray(evolved.payload.fields)) {
            evolved.payload.fields = [];
        }
    }
