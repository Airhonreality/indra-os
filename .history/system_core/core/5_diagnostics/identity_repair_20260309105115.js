  ...
  while (subFolders.hasNext()) {
    ...
    while (files.hasNext()) {
      ...
      try {
        ...
        // --- 3.5 Normalización de Payload (ADR-008 Schema Compliance) ---
        if (content.class === 'DATA_SCHEMA') {
          if (!content.payload) content.payload = {};

          // Función recursiva para sanear el AST de campos (Deep Audit)
          const repairFields = (fields) => {
            if (!Array.isArray(fields)) return [];
            return fields.map(f => {
              let fChanged = false;
              // Sinceridad Radical: si no hay label/id, se infieren del nombre antiguo UNA SOLA VEZ
              ...
              if (f.name) { delete f.name; fChanged = true; }
              if (f.key) { delete f.key; fChanged = true; }

              const newField = { ...f, id, alias, label };
              if (f.children) newField.children = repairFields(f.children);
          ...
          const rawFields = content.payload.fields || content.fields || content.raw?.fields || [];
          const repaired = repairFields(rawFields);

          if (JSON.stringify(content.payload.fields) !== JSON.stringify(repaired)) {
            console.log(`    🧬 Sanitizando AST de campos para: ${file.getName()} (${repaired.length} campos)`);
            content.payload.fields = repaired;
            changed = true;
          }

          if (content.fields) { delete content.fields; changed = true; }
        }

        // --- 4. Depuración de Legacy (Capa Root) ---
        if (content.name) {
