    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setStream(prev => prev.map(v => ({
                ...v,
                value: v.type === 'number' ? (v.value + (Math.random() - 0.5) * 0.01).toFixed(3) : v.value
            })));
        }, 2000);
        ...
        const fetchContext = async () => {
            try {
                ...
                const data = [
                    ...
                    { id: 'isk.z_index', label: 'Layer Depth', type: 'number', value: 5, source: 'LAYOUT' }
                ];
                setStream(data);
                setLoading(false);
