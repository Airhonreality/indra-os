...
function _setupAPIGatewayTests() {
  ...
  const mocks = {
    ...
    mockPublicApi: {
      ...
      getSystemContracts: function () { return { contracts: { publicApi: { label: 'PublicAPI' } } }; }
    },
    mockFlowRegistry: {
      label: 'FlowRegistry',
  ...
  globalThis._assembleExecutionStack = () => ({
    ...
    public: mocks.mockPublicApi, // Alias requerido por HttpEntrypoint.gs (line 241)
...
function testAPIGateway_Api_debeDevolverContratosConGetSystemContracts() {
  ...
  try {
    ...
    // DEBUG: console.log(JSON.stringify(responseContent));
    assert.isTrue(responseContent.success);
    assert.hasProperty(responseContent.result, 'contracts'); // getSystemContracts returns { contracts: ... }
    assert.hasProperty(responseContent.result.contracts, 'publicApi');
    return true;
  } finally {
    _teardownAPIGatewayTests(setup.originals);
  }
...
function testAPIGateway_Api_debeFallarSiElExecutorOElMetodoSonInvalidos() {
  ...
  try {
    ...
    assert.areEqual('EXPOSURE_BLOCK', response1.error.code);
    ...
    assert.areEqual('EXPOSURE_BLOCK', response2.error.code);
    return true;
  } finally {
    _teardownAPIGatewayTests(setup.originals);
  }
...
function testAPIGateway_Api_debeRechazarMetodosInternos() {
  ...
  try {
    ...
    assert.areEqual('EXPOSURE_BLOCK', response.error.code);
    return true;
  } finally {
    _teardownAPIGatewayTests(setup.originals);
  }
