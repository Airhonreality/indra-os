...
function createFlowCompiler({ errorHandler, blueprintRegistry }) {
  if (!errorHandler) throw new Error("FlowCompiler: errorHandler is required");

  /**
   * Compila un flujo basado en su topología.
   * AXIOMA: L5 Structural Verification (Morphism Alignment).
   * 
  ...
  function compile(flow, nodeRegistry = {}) {
    ...
    connections.forEach(conn => {
      ...
      if (blueprintRegistry && nodeRegistry && fromPort && toPort) {
