...
function createFlowRegistry({ manifest, driveAdapter, configurator, errorHandler }) {
  ...
  const anchorPropertyKey = manifest.ANCHOR_PROPERTY;
  if (!anchorPropertyKey) {
    throw errorHandler.createError(
      'CONFIGURATION_ERROR',
      'Manifest is missing ANCHOR_PROPERTY'
    );
  }

  // AXIOMA: Soberanía de Anclaje
  ...
  function ensureFlowsFolderResolved() {
    ...
    // El anclaje ya fue validado en el constructor

    // Obtener la ruta de la carpeta de flujos del manifiesto
    const flowsFolderPath = manifest.DRIVE_SCHEMA?.FLOWS?.PATH;
    if (!flowsFolderPath) {
      throw errorHandler.createError(
        'CONFIGURATION_ERROR',
        'Manifest is missing DRIVE_SCHEMA.FLOWS.PATH'
      );
    }
  ...
  function getFlow(rawArg) {
    // --- RESOLUCIÓN SOBERANA DE ARGUMENTO ---
