...
function _setupConfiguratorTests() {
  ...
  const mocks = {
    ...
    mockProperties: {
      getProperty: (key) => {
        ...
        console.log(`[MockProps] getProperty '${key}' -> '${val}'`);
        return val;
      },
      setProperty: (key, value) => {
        console.log(`[MockProps] setProperty '${key}' = '${value}'`);
        storage[key] = String(value);
      },
      deleteProperty: (key) => { delete storage[key]; },
      getProperties: () => ({ ...storage }),
      deleteAllProperties: () => { for (const k in storage) delete storage[k]; }
    },
    _state: {
      storage
    }
  ...
  globalThis.PropertiesService = {
    ...
    getUserProperties: () => mocks.mockProperties,
    getDocumentProperties: () => mocks.mockProperties
  };
...
function testConfigurator_CicloDeVida_debeAlmacenarYRecuperarUnParametro() {
  ...
  try {
    ...
    if (testValue !== retrievedValue) {
      throw new Error(`Assertion Failed: Expected '${testValue}' but got '${retrievedValue}'`);
    }
    return true;
  } finally {
...
function testConfigurator_RetrieveParameter_debeRetornarNullParaUnaClaveInexistente() {
  ...
  try {
    ...
    if (retrievedValue !== null) {
      throw new Error(`Assertion Failed: Expected null but got '${retrievedValue}' (Type: ${typeof retrievedValue})`);
    }
    return true;
  } finally {
    _teardownConfiguratorTests(setup.originals);
...
function testConfigurator_GetConfigurationStatus_debeRetornarCompletoSiTodasLasClavesExisten() {
  ...
  try {
    const configurator = createConfigurator({ manifest: mockManifest, errorHandler: setup.mocks.mockErrorHandler });
    // Use public API to set up state to ensure prefix correctness
    configurator.storeParameter({ key: 'core:KEY_1', value: 'value1' });
    configurator.storeParameter({ key: 'core:KEY_2', value: 'value2' });
...
function testConfigurator_BackwardCompatibility_debeRedirigirATokenManager() {
  ...
  try {
    ...
    // Se usa storeParameter para simular que ya existe, evitando prefijos manuales erroneos
    try {
    ...
    if (valueDirect !== 'direct-ps-value') {
      throw new Error(`Assertion Failed: Expected 'direct-ps-value' but got '${valueDirect}'`);
    }
