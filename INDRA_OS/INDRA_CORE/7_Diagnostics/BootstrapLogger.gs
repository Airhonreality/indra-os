...
const BootstrapLogger = {
  ...
  log: function(message) {
    try {
      ...
      const logSheetId = properties.getProperty('AXIOM_CORE_AUDIT_LOG_SHEET_ID');
      
      if (!logSheetId) {
        // Si no se puede encontrar el ID, es imposible loguear en el Sheet.
        // Se recurre al log estándar de GAS como último recurso.
        console.error("BootstrapLogger CRITICAL: La propiedad 'AXIOM_CORE_AUDIT_LOG_SHEET_ID' no está configurada. No se puede escribir en el Audit Log Sheet.");
  ...
  test: function() {
    ...
    if (logSheetId) {
        ...
        console.error("La prueba no pudo escribir en el Sheet porque la propiedad 'AXIOM_CORE_AUDIT_LOG_SHEET_ID' no está configurada.");
    }
  }
};
