...
¿Panel JSON Espejo incluido? Sí. Es vital para la transparencia cognitiva. El usuario debe ver cómo sus palabras se transforman en código.
9. Vínculo de Integridad Axiomática (The Core Contract)
--------------------------------------------------
El copiloto es la manifestación de la inteligencia orquestada del sistema.
...
- `analyzeContext(state)`: Introspección proactiva del estado de la aplicación.
- `suggestNextStep()`: Predicción de flujos basada en el historial de transacciones.

---
**Veredicto Final del Arquitecto**: Este rediseño transforma al OMD-04 en un **Garante de la Gnosis**, permitiendo al usuario navegar la complejidad del sistema mediante un diálogo técnico-soberano de alto nivel.
Se sitúa como una pestaña o un panel colapsable dentro del mismo módulo.
