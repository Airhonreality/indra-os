...
powershell -Command "$url = Get-Content '..\..\.gas-url.txt' -Raw; if($url) { \"VITE_CORE_URL=$url`nVITE_SYSTEM_TOKEN=c6c4625b-267c-4d42-a351-86540c5a1b56`nVITE_CLIENT_NAME=INDRA_V2_STARK_ARCHITECT`nVITE_VERSION=2.0.0\" | Out-File -FilePath .env -Encoding utf8; echo '✅ URL y Token Sincronizados.' } else { echo '⚠️ No se encontro .gas-url.txt, usando valores previos.' }"
