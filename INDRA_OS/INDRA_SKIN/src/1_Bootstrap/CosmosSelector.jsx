...
import contextClient from '../core/kernel/ContextClient.js';
import ProjectionMatrix from '../core/kernel/ProjectionMatrix.jsx';
import ArtifactExplorer from './ArtifactExplorer.jsx';
...
const CosmosSelector = () => {
    ...
    // AXIOMA: Autodiscovery on mount (ADR-022 Continuity)
    useEffect(() => {
        if (availableCosmos.length === 0 && discoveryStatus === 'IDLE') {
            execute('START_DISCOVERY');
        }
    }, [availableCosmos.length, discoveryStatus]);
    ...
    const handleCreateCosmos = async (payload) => {
        setLocalError(null);
        // AXIOMA: Spinner interno local para no bloquear la app
        // removemos execute('START_DISCOVERY') para evitar doble batching que cruza las lecturas en la nube.

        try {
            const identity = payload.identity || { label: payload.label, description: payload.description };

            // ID determinista CON prefijo 'cosmos_' para que el backend lo identifique como nuevo
            ...
            if (response && response.success && (response.id || response.new_id || response.fileId)) {
                ...
                // Actualizar inmediatamente el registro visible para que el nuevo Cosmos se manifieste en la lista actual
                const newCosmos = {
                    id: realId,
                    name: identity.label ? `${identity.label}.cosmos.json` : "mi_universo.cosmos.json",
                    identity: { label: identity.label || "Sin Título", description: identity.description },
                    indx_schema: 'COSMOS_V1',
                    discovery: { status: 'ARTIFACT_INDEXED', is_json: true }
                };
                execute('UPDATE_COSMOS_REGISTRY', [newCosmos, ...availableCosmos]);
    ...
    if (isDiagnosticMode) {
        return (
            <div className="w-full h-full relative">
                <ArtifactExplorer onClose={() => setIsDiagnosticMode(false)} />
            </div>
        );
    }
    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-center p-8 bg-transparent">
            <div className="max-w-2xl w-full flex flex-col gap-8">
                {/* Header */}
                <div className="flex flex-col gap-2 border-b border-[var(--axiom-glass-border)] pb-6 relative">
                ...
                <div className="flex-1 min-h-[300px] overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-3">
                    {isLoading && availableCosmos.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-64 border border-[var(--axiom-glass-border)] rounded-xl bg-black/10 backdrop-blur-sm">
                            <AxiomaticSpinner size={48} label="Escaneando Registro de Flujos" />
                        </div>
                    ) : availableCosmos.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-48 border border-[var(--axiom-glass-border)] rounded-xl text-[var(--text-dim)] gap-4 border-dashed">
                            ...
                            <div
                                ...
                                className={`
                                    ...
                                    ${selectedCosmosId === artifact.id
                                        ...
                                        : 'bg-[var(--bg-secondary)] border-[var(--axiom-glass-border)] hover:border-[var(--text-dim)] shadow-sm'
                                    }
                                `}
                            >
                                <div className="flex items-center gap-5">
                                    <div className={`
                                        ...
                                        ${selectedCosmosId === artifact.id ? 'bg-[var(--accent)] text-black border-transparent' : 'bg-[var(--bg-deep)] border-[var(--axiom-glass-border)] text-[var(--accent)]'}
                                    `}>
                ...
                <div className="flex gap-4 pt-4">
                    <button
                        ...
                        disabled={isLoading}
                        className="px-6 py-3 rounded-xl font-mono text-[10px] uppercase tracking-[0.2em] bg-[var(--bg-secondary)] border border-[var(--axiom-glass-border)] text-[var(--text-dim)] hover:text-[var(--text-primary)] transition-all shadow-sm"
                    >
