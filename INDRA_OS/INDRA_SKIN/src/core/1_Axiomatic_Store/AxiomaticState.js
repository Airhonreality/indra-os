...
const useAxiomaticState = create(subscribeWithSelector((set, get) => ({
     ...
     */
    igniteHash: async () => {
        // Marcar como inicializado inmediatamente — no bloquear el auto-mount
        set((state) => ({
            session: { ...state.session, hashInitialized: true }
        }));

        // Lectura de hash posterior (informativo, no bloqueante)
        try {
            let hash = await AxiomaticDB.getItem('AXIOM_REVISION_HASH');
            if (!hash) {
                hash = localStorage.getItem('AXIOM_REVISION_HASH');
            }
            if (hash) {
                set((state) => ({
                    session: { ...state.session, currentRevisionHash: hash }
                }));
            }
        } catch (e) {
            console.warn("[AxiomaticState] Hash recall failed (non-blocking):", e);
        }
    },
    ...
    updateRevisionHash: (hash) => {
        ...
        localStorage.setItem('AXIOM_REVISION_HASH', hash);
        AxiomaticDB.setItem('AXIOM_REVISION_HASH', hash).catch(e =>
    ...
    updateSyncStatus: (status, error = null) => {
        set((state) => {
            ...
            if (status === 'RETRY' || status === 'OFFLINE') {
                const currentAttempts = state.session.failedSyncAttempts || 0;
                updates.session.failedSyncAttempts = currentAttempts + 1;
...
// No exposición global window.AXIOM_AXIOMATIC_STATE
