...
function createCoreOrchestrator({ 
  ...
  if (!constitution || !constitution.systemLimits) {
    throw new Error('CoreOrchestrator: constitution con systemLimits es requerido.');
  }
  ...
  const { maxRetries = 3, initialBackoffMs = 100 } = constitution.systemLimits;

  // ============================================================
  // FASE 2: CONSTANTES DE CLASIFICACIÓN
  // ============================================================
  ...
  function _isSpecialNode(step) {
    if (!constitution || !constitution.orchestratorMetadata || !constitution.orchestratorMetadata.specialNodes) {
      return false;
    }
    const specialNodes = constitution.orchestratorMetadata.specialNodes;
    const methods = specialNodes[step.adapter];
  ...
  function executeNode(step, flowContext) {
    ...
    // [V5.0] GRAPH-AWARE: Resolver entradas desde la topología (cables) antes de renderizar
  ...
  /**
   * [V5.0] GRAPH-AWARE: Resuelve el cableado entre nodos.
  ...
  // ============================================================
  // FASE 4: PROCESAMIENTO DE FLUJOS
  // ============================================================
  ...
  function processSteps(steps, context, connections = []) {
    ...
    // AXIOMA v8.0: State Snapshotting (Industrial Traceability)
    if (!currentContext._history) currentContext._history = [];
    const _snapshotLimit = 5; // v9.0: Más restrictivo para proteger el heap
    ...
    for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
      ...
      if (step.try && Array.isArray(step.try)) {
        ...
        // [V5.0] Almacenamiento AUTOMÁTICO por ID (Persistencia de la Realidad)
        ...
        // AXIOMA v8.0: Registrar snapshot del estado actual
        try {
          const snapshot = {
            stepId: stepId,
  ...
  function executeFlow(flow, initialContext) {
    ...
    // AXIOMA v6.0: Compilación Topológica Automática (Compiled Mode)
    ...
    // [V5.0] Inicializar mapa de nodos y extraer conexiones
    const flowContext = { 
      nodes: {}, 
      ... (initialContext || {}) 
    };
