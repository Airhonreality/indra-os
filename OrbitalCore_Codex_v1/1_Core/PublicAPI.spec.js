...
function _setupPublicAPITests() {
  ...
  const mocks = {
    ...
    mockSchemaRegistry: {
      label: "Mock Schema Registry",
      description: "Mocked schema registry for tests.",
      _callLog: [],
...
// ============================================================
// SUITE DE TESTS PARA PUBLICAPI (ACTUALIZADA - FASE L)
// ============================================================
...
function _createMockedPublicApi(setup) {
  return createPublicAPI({
    ...
    schemaRegistry: setup.mocks.mockSchemaRegistry,
...
function testPublicAPI_PhaseL1_BootstrapHalt() {
  ...
  try {
    // Simulamos fallo crítico en el gatekeeper
    setup.mocks.mockGatekeeper.validateAllContracts = () => ({
      isValid: false,
      criticalErrors: ['TIER 1 VIOLATION: MasterLaw desincronizado'],
      errors: [],
      warnings: [],
      auditedModules: 1
    });
    ...
    assert.throws(() => {
      _createMockedPublicApi(setup);
    }, '[ARCHITECTURAL_HALT]');

    console.log("✅ testPublicAPI_PhaseL1_BootstrapHalt passed (System halted as expected).");
    return true;
  } finally {
...
function testPublicAPI_PhaseL2_SecurePipe_StructuralBlock() {
  ...
  try {
    ...
    // Intentamos llamar a un método con input inválido (detectado por nuestro mockSchemaRegistry)
    assert.throws(() => {
      publicApi.scanArtifacts({ folderId: 'INVALID' });
    }, 'STRUCTURAL_BLOCK');
    ...
    assert.isTrue(setup.mocks.mockSchemaRegistry._callLog.length > 0, "SchemaRegistry debió ser consultado para validación.");
...
function testPublicAPI_PhaseL4_GovernanceReport() {
  ...
  try {
    ...
    assert.areEqual("100.00", report.coherenceIndex, "Coherencia debe ser 100% con mocks limpios");
...
function testPublicAPI_PhaseL5_SemanticAffinity() {
  ...
  try {
    ...
    assert.arrayLength(setup.mocks.mockGatekeeper._callLog.filter(c => c.method === 'getAffinity'), 1, "Gatekeeper.getAffinity debió ser consultado.");
