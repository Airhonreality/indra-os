...
function _setupSystemInitializerTests() {
  ...
  const mocks = {
    mockManifest: {
      anchorPropertyKey: 'ORBITAL_CORE_ROOT_ID',
      driveSchema: {
        rootFolderName: 'OrbitalCore',
        jsonFlowsFolder: { path: 'Flows' },
      },
      sheetsSchema: {
        jobQueue: { propertyKey: 'JOB_QUEUE_ID', sheetName: 'JobQueue', header: ['id'] },
      },
      requiredConnections: { // Requerido para migración de tokens
...
function testSystemInitializer_Verificacion_noDebeHacerNadaSiTodoEstaConfigurado() {
  ...
  try {
    ...
    setup.mocks._state.storedProperties['system_folder_jsonFlowsFolder_id'] = 'folder-id';
