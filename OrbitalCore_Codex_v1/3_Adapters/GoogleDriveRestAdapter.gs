...
function createGoogleDriveRestAdapter({ errorHandler, tokenManager }) {
  ...
  function find(payload) {
    ...
    const endpoint = `${BASE_URL}?q=${encodeURIComponent(sanitizedQuery)}&fields=files(id,name,mimeType,modifiedTime)`;
    const result = _callApi(endpoint, { method: 'get' }, payload);
    
    return {
      foundItems: (result.files || []).map(f => ({
        id: f.id,
        name: f.name,
        mimeType: f.mimeType,
  ...
  const schemas = {
    find: {
      ...
      io_interface: {
        ...
        outputs: { 
          foundItems: { type: "array", io_behavior: "STREAM", description: "Collection of discovered resource metadata." } 
        }
      }
    },
    retrieve: {
  ...
  return Object.freeze({
    label: "External Storage Orchestrator",
    description: "Industrial bridge for external Google Drive account orchestration via V3 REST API.",
