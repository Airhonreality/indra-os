...
const _TestConfig = {
  ...
  runFile: {
    ...
    IndraAdapter: true, // ✅ NEW: Cognitive Sensing
    ...
    DifferentialTesting: true, // ✅ NEW: Garantiza integridad de autoproyección UI
    ContractCompliance: true, // 🕵️ NEW: Auditoría Global de Cumplimiento de Contratos
    Integration: true, // 🔗 NEW: Pruebas de integración Core-to-Core
...
function RunAllTests() {
  ...
  try {
    ...
    if (stack) {
       ...
       console.log('ContractGatekeeper ha certificado la pureza estructural.');
    } else {
...
function _discoverTestFunctions() {
  ...
  const testNames = allNames.filter(name => {
    ...
    const configMap = {
      ...
      'testIndraAdapter': _TestConfig.runFile.IndraAdapter || _TestConfig.runLayer.Core,
