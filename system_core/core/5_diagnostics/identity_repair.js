    ...
    while (files.hasNext()) {
      ...
      try {
        ...
        if (content.class === 'DATA_SCHEMA') {
          ...
          if (!content.payload.fields || !Array.isArray(content.payload.fields)) {
            // Intentar recuperar de raw.fields o root fields
            const recoveredFields = (content.raw && content.raw.fields) || content.fields || [];
            console.log(`    🧬 Normalizando Payload para DATA_SCHEMA: ${file.getName()} (${recoveredFields.length} campos encontrados)`);
            content.payload.fields = Array.isArray(recoveredFields) ? recoveredFields : [];
            changed = true;

            // Limpieza de campos redundantes en root si existen
            if (content.fields) delete content.fields;
          }
        }
