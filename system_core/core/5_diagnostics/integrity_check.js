...
function runIntegrityCheck() {
    ...
    const checkFolder = (folder) => {
        ...
        while (files.hasNext()) {
            ...
            if (size === 0) {
                const zombieTrace = { id: file.getId(), name: file.getName(), folder: folder.getName() };
                report.zombies.push(zombieTrace);
                report.summary.errors++;
                logError(`[integrity] MATTER_ZOMBIE (0 bytes): "${file.getName()}"`, zombieTrace);
                continue;
            }
            ...
            try {
                ...
                const corruptTrace = { id: file.getId(), name: file.getName(), folder: folder.getName(), error: e.message };
                report.corrupt.push(corruptTrace);
                report.summary.errors++;
                logWarn(`[integrity] MATTER_CORRUPT (Bad JSON): "${file.getName()}"`, corruptTrace);
            }
        }
